    ...
    <div>
      <div className='flex'> {/* input-box */}
        <label className="relative block ml-2 w-4/5">
          ...
          <input className="w-full placeholder:italic placeholder:text-slate-400 block bg-yellow-50 border border-slate-300 rounded-full py-2 px-5 shadow-sm focus:outline-none focus:border-orange-300 focus:ring-orange-300 focus:ring-1 sm:text-sm disabled:bg-slate-300 disabled:cursor-not-allowed" placeholder="prashn puchiye ..." type="text" name="search" value={inputValue} onChange={handleChange} disabled={isSearchDisabled}
            onKeyDown={
              (e) => {
                if (e.key === 'Enter') {
                  handleSubmit();
